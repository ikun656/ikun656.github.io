...
(function () {
    ...
    var POLICY_TEXT = [
        ...
        '局域网传文件：基于浏览器 WebRTC 技术，在两台设备之间建立点对点连接后直接传输文件。连接需要先由双方互相交换一次邀请码/应答码（含网络候选信息）来协商，文件数据仅在两端之间直传，不经过本站或任何中转服务器；我们既收不到也无法看到传输内容。',
        '',
        ...
        '本站的代码（HTML / CSS / JavaScript）以公开方式托管在 GitHub 上，供个人下载、访问与离线查看参考，但不得进行任何修改、改编或二次开发，也不得重新分发、公开展布或用于商业用途。本内容受本站自定义许可证（详见仓库 LICENSE 文件）约束并保留一切权利；页面中的品牌标识与原创内容版权归原作者所有；各工具所涉及的第三方软件，其版权归相应上游项目所有，请遵守各自的许可协议。',
        '',
